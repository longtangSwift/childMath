...
class MathBrain{
    ...
    func formProblem (operations: String)-> (num1: Int, num2: Int, op: String) {
        ...
        while (num1 == 1 && randX.random(100) > 9){num1 = rand}
        while num2 == 1 && randX.random(100) > 9{num2 = rand}
    ...
    func shouldWeGo(num1: Int, num2: Int, op: String)->Bool{
        let showProbability = 3; //we only have a low chance of showing any one problem
        var adjust1 = 0; var adjust2 = 0
        for i in reDoArr{
            if i.num1 == num1 && i.num2 == num2 && i.op == op {print("Wow! We hit an adjuster!"); adjust1 += 33; adjust2 += 55}
        }
